...
STEP 1 — Read SNOW ticket:
  ...
  Remember: sys_id, short_description, description. Extract cost_center from description \
(look for "Cost center:" or "CC-" patterns).

STEP 2 — Azure inventory (OPTIONAL):
  Only if azure__ tools appear in your tool list: call the resource group list tool.
  If no azure__ tools exist, skip this step entirely.
...
async def _dispatch(
    ...
    # Safety: inject Terraform content if the agent forgot it in push_files
    if state["terraform_state"] and "github__" in fn_name:
...
async def provision_from_ticket(
    ...
    with MultiMCPClient(configs) as mcp:
        ...
        # Mutable state shared across the streaming event handler
        state: Dict[str, Any] = {
            "iterations": 0,
            "trace": [],
        ...
        async with AgentsClient(
            ...
            agent = await agents_client.create_agent(
                model=openai_settings.deployment_name or "gpt-4.1",
                name=f"snow-tf-{ticket_id[:20]}",
                instructions=filled_system_prompt,
                tools=[{"type": "function", "function": d} for d in tool_defs],
            )
            try:
                ...
                async with agents_client.create_stream(
                    ...
                    async for event_type, event_data, _ in stream:
                        if isinstance(event_data, SubmitToolOutputsAction):
                            outputs = []
                            for tc in event_data.submit_tool_outputs.tool_calls:
                                ...
                                _telemetry.track_llm_call(
                                    ...
                                    action_returned=fn_name,
                                    duration_seconds=llm_timer.elapsed,
                                )
                                outputs.append({"tool_call_id": tc.id, "output": output})

                            await stream.submit_tool_outputs(tool_outputs=outputs)

                        # Exit early once complete_provisioning or auto-complete fires
